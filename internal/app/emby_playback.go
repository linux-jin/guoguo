...
func (app *UIApp) acquireEmbyPlayback(writer http.ResponseWriter, request *http.Request, id, chapter, sessionID string, legacy bool, forceProxyOptions ...bool) *playbackSession {
	forceProxy := len(forceProxyOptions) > 0 && forceProxyOptions[0]
	...
	if sessionID != "" {
		...
		for _, candidate := range app.playbacks {
			if candidate.emby && candidate.embyLegacy == legacy && candidate.embyForceProxy == forceProxy && candidate.accountID == owner && candidate.dramaID == id && len(candidate.tasks) == 1 && candidate.tasks[0].Chapter.ID == chapter && candidate.mediaError == nil {
				session = candidate
				break
			}
		}
	...
	if session == nil {
		...
		session = &playbackSession{id: sessionID, accountID: owner, dramaID: id, emby: true, embyLegacy: legacy, embyForceProxy: forceProxy, run: 1,
