...
function applyWatchOnly(){
  if(!config.watchOnly)return;
  document.body.classList.add('watch-only');
  const box=$('prefetchNextEpisode');
  if(box){box.checked=false;box.disabled=true;}
  if(window.JukuDialogs?.page()==='downloads')window.JukuDialogs.navigate('library');
}
async function loadConfig(){try{config=await api('/api/ui/config');refreshConfigFields();applyWatchOnly();}catch(error){$('configText').textContent='配置读取失败：'+error.message;}}
