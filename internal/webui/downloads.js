import { withFocus, readPreference, savePreference } from './ui-core.js';
...
export function createDownloads(app) {
  ...
  let pollTimer, pollPromise, batch = false;
...
function renderTasks(next) {
  ...
  $('taskCount').textContent = all.length ? visibleGroups.length + ' 部' : '';
  const active = tasks.filter(task => ['running', 'queued', 'parsing'].includes(task.status)).length;
  $('downloadActiveCount').textContent = active;
  $('downloadCompleteCount').textContent = tasks.filter(task => task.status === 'success').length;
  $('downloadWrittenBytes').textContent = formatBytes(tasks.reduce((total, task) => total + number(task.downloadedBytes), 0));
  document.querySelectorAll('[data-download-count]').forEach(node => {node.textContent = active; node.hidden = !active;});
  ...
  withFocus(groupsEl, () => {
    ...
    for (const group of visibleGroups) {
      ...
      if (old?.renderKey === key) {fragment.appendChild(old); continue;}
...
async function pollTasks() {
  ...
  pollPromise = (async () => {
    try {
      ...
      renderFFmpeg(result.ffmpeg);
      $('taskError').textContent = '';
    } catch (error) {
      $('taskError').textContent = '任务刷新失败：' + error.message;
    } finally {taskRequest = false;}
  })();
  return pollPromise;
}
...
async function enqueue(ids) {
  ...
  try {
    await post('/api/ui/download', {ids});
    setMessage('已加入下载队列 · ' + ids.length + ' 部', false, {label: '查看下载', run: () => window.JukuDialogs.navigate('downloads')});
    await pollTasks();
    return true;
...
function schedule() {
  ...
  pollTimer = setTimeout(async () => {await pollTasks(); schedule();}, document.hidden ? 30000 : 2000);
}
...
return {init, refresh: pollTasks, render: () => renderTasks(tasks), enqueue, all: () => tasks};
