...
export function createLibrary(app) {
  ...
  let searchController, searchSequence = 0, cardRender = 0, batch = false, filterTimer;
...
function filteredDramas(){ const keyword=$('searchInput').value.trim().toLowerCase();const source=$('sourceSelect').value;const channel=$('channelSelect').value;return dramas.filter(dr=>{if(source&&sourceKey(dr)!==source)return false;const cat=categoryName(dr);if(channel&&cat!==channel)return false;return !keyword||dramaSearchText(dr).includes(keyword)||onlineSearchQuery===keyword&&onlineSearchIDs.has(dr.id);}); }

function rebuildSources(){ const known=['huangguo','huangdou','hongguo'];const extra=Array.from(new Set(dramas.map(sourceKey))).filter(value=>!known.includes(value)).sort();rebuildOptions($('sourceSelect'),known.concat(extra),'全部站源',sourceLabel,false); }
...
function updateLibraryButton() {
  const source = $('sourceSelect').value, hongguo = !source || source === 'hongguo';
...
function metadataPriorityIDs(){const pending=new Set(dramas.filter(drama=>!drama.sortMetadata||drama.sortMetadata.version!==(['huangguo','huangguoai','huangguoai.com'].includes(String(drama.source||drama.id).split(':')[0])?2:1)||sourceKey(drama)==='hongguo'&&!coverURL(drama)&&!drama.sortMetadata.coverChecked).map(drama=>drama.id));return visibleIDs.filter(id=>pending.has(id)).slice(0,40);}

function updateRefreshLabel(){ const source=$('sourceSelect').value;const hongguo=!source||source==='hongguo';updateLibraryButton();$('onlineSearchBtn').hidden=!hongguo;$('searchInput').placeholder='搜索剧名、简介或标签';$('onlineSearchBtn').title='联网搜索红果，也可按回车'; }
...
function renderCard(drama) {
  ...
  actions.append(app.following.saveButton(drama.id, title), download);
...
function renderDramas() {
  ...
  const list = window.JukuLibrarySort.sortDramas(filtered, mode);
  sortMessage = window.JukuLibrarySort.summary(filtered, mode);
  $('sortSelect').title = '缺少数据的剧集排在后面；不同站源的统计口径可能不同。' + sortMessage;
  visibleIDs = list.map(drama => drama.id);
  $('dramaCount').textContent = list.length === dramas.length ? list.length + ' 部' : list.length + ' / ' + dramas.length + ' 部';
  updateDramaSelection();
  updateLibraryStatus();
  ...
  if (!sameOrder) cards.replaceChildren();
  const version = ++cardRender;
  let offset = 0;
  cards.setAttribute('aria-busy', String(list.length > 80));
  function appendBatch() {
    if (version !== cardRender) return;
    const end = Math.min(offset + 80, list.length);
    for (; offset < end; offset++) {
      const drama = list[offset];
      let card = previous.get(drama.id);
      ...
      if (sameOrder) {if (cards.children[offset] !== card) cards.replaceChild(card, cards.children[offset]);}
      else cards.appendChild(card);
    }
    if (offset < list.length) setTimeout(appendBatch, 0);
    else {cards.setAttribute('aria-busy', 'false'); refreshFollowing();}
  }
  appendBatch();
  if (!list.length) {
    const state = element('div', 'empty');
...
async function loadDramas(update) {
  ...
  try {
    ...
    if (Array.isArray(result.data)) {
      dramas.length = 0;
      byID.clear();
      for (const drama of result.data) {dramas.push(drama); byID.set(drama.id, drama);}
      rebuildSources();
...
function refreshFilterSummary() {
  ...
  $('resetFiltersBtn').hidden = !$('searchInput').value && !$('channelSelect').value && $('sortSelect').value === 'default' && ['', 'hongguo'].includes($('sourceSelect').value);
...
function init() {
  ...
  $('sourceSelect').value = ['', 'hongguo', 'huangguo', 'huangdou'].includes(saved.source) ? saved.source : 'hongguo';
...
return {init, get: id => byID.get(id), all: () => dramas, render: renderDramas, refreshFollowing, refresh: () => {libraryRevision = 0; return loadDramas(false);}, retryCovers: () => cards.querySelectorAll('.card').forEach(card => card.retryCover?.())};
